...
import { Search, Info, CheckCircle2, Trash2, FileCode, ArrowRight, Check, Loader2, Sparkles, Film, TrendingUp, Calendar, AlertTriangle, ChevronDown, ChevronUp, Plus, Award } from 'lucide-react';
import { ToolType, FileItem, ProgressInfo } from '../../types';
import { analyzeAssetSeo } from '../../seoMarketEngine';
...
const ProjectKeywordList: React.FC<KeywordListProps> = ({
  ...
  const [copied, setCopied] = React.useState(false);
  const [showSeoPanel, setShowSeoPanel] = React.useState(true);

  const seoResult = React.useMemo(() => {
    return analyzeAssetSeo(title || '', keywords);
  }, [title, keywords]);

  const isIndo = t && (t.language === 'Bahasa' || t.title_label?.toLowerCase().includes('judul'));
  ...
  return (
    <div className="space-y-1.5 font-sans">
      ...
      {/* 📊 MARKET SEO INTELLIGENCE DASHBOARD PANEL */}
      <div className="mt-3 border border-slate-200/85 dark:border-slate-800 rounded-[1.5rem] bg-slate-50/50 dark:bg-black/10 overflow-hidden font-sans">
        {/* Toggle Header */}
        <button
          type="button"
          onClick={() => setShowSeoPanel(!showSeoPanel)}
          className="w-full flex items-center justify-between p-3 text-left hover:bg-slate-100/50 dark:hover:bg-white/5 transition-colors cursor-pointer"
        >
          <div className="flex items-center space-x-2">
            <span className="text-sm">📊</span>
            <span className="text-[10px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">
              {isIndo ? 'DASHBOARD OPTIMASI SEO PASAR' : 'MARKET SEO OPTIMIZATION DASHBOARD'}
            </span>
          </div>
          <div className="flex items-center space-x-2">
            {/* Score pill */}
            <span className={`px-2 py-0.5 rounded-full text-[9px] font-extrabold flex items-center gap-1 border ${seoResult.ratingsColor}`}>
              <TrendingUp className="w-2.5 h-2.5" />
              {isIndo ? `Skor SEO: ${seoResult.score}/100` : `SEO Score: ${seoResult.score}/100`} ({seoResult.rating})
            </span>
            {showSeoPanel ? (
        ...
        {showSeoPanel && (
          <div className="p-3 border-t border-slate-100 dark:border-slate-800 space-y-3.5 animate-in fade-in duration-200">
            {/* Score progress bar & Diagnostics checklist */}
            <div className="space-y-2">
              <div className="w-full bg-slate-200 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden">
                <div
                  className="bg-gradient-to-r from-violet-500 to-indigo-600 h-full transition-all duration-500 rounded-full"
              ...
              <div className="space-y-1.5">
                {(isIndo ? seoResult.feedbackIndo : seoResult.feedback).map((item, idx) => (
                  <div key={idx} className="flex items-start text-[10px] leading-tight font-bold text-slate-600 dark:text-slate-400">
                    <span className="mr-1.5 shrink-0 text-violet-500 text-xs">⚡</span>
                    <span>{item}</span>
                  </div>
                ))}
              </div>
            ...
            {/* Keyword Associations (Co-occurrence Suggestions) */}
            {seoResult.suggestedCoOccurrences.length > 0 && (
              <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1.5">
                <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">
                  <span>🔗 {isIndo ? 'REKOMENDASI HUBUNGAN KATA KUNCI (CO-OCCURRENCE)' : 'SUGGESTED ASSOCIATIONS (CO-OCCURRENCE)'}</span>
                  <span className="text-[8px] lowercase font-normal italic">
                    {isIndo ? 'klik untuk menambah tag' : 'click to quickly add'}
                  </span>
                </div>
                <div className="flex flex-wrap gap-1.5 pt-0.5">
                  {seoResult.suggestedCoOccurrences.map((tag) => (
                    <button
                      key={tag}
                      type="button"
                      onClick={() => {
                      ...
                      className="inline-flex items-center px-2 py-0.5 rounded text-[9.5px] font-bold bg-[#7c3aed]/5 hover:bg-[#7c3aed]/15 text-[#7c3aed] border border-[#7c3aed]/20 transition-all cursor-pointer shadow-sm active:scale-95"
                    >
                      <Plus className="w-2.5 h-2.5 mr-0.5 shrink-0 text-[#7c3aed]" />
                      {tag}
                    </button>
                  ))}
                </div>
            ...
            {/* Holiday Campaign Boost (Seasonal Events) */}
            {seoResult.relevantSeasonalEvents.length > 0 && (
              <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-2">
                {seoResult.relevantSeasonalEvents.map((event) => {
                  const eventTagsNotAdded = event.popularKeywords.filter(
                    (tag) => !keywords.map((k) => k.toLowerCase().trim()).includes(tag.toLowerCase().trim())
                  );

                  return (
                    <div key={event.id} className="p-2.5 rounded-xl bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/20 space-y-1.5">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-1.5">
                          <Calendar className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400 shrink-0" />
                          <span className="text-[10px] font-black uppercase text-amber-700 dark:text-amber-400 tracking-wide">
                            🔥 {isIndo ? event.nameIndo : event.name} ({isIndo ? 'Puncak:' : 'Peak:'} {isIndo ? event.peakDemandIndo : event.peakDemand})
                          </span>
                        </div>
                        {eventTagsNotAdded.length > 0 && (
                          <button
                            type="button"
                            onClick={() => {
                              const uniqueTags = event.popularKeywords.filter(
                                (tag) => !keywords.map((k) => k.toLowerCase().trim()).includes(tag.toLowerCase().trim())
                              );
                              if (uniqueTags.length > 0) {
                                onChange([...keywords, ...uniqueTags]);
                              }
                            }}
                            className="text-[9px] font-black uppercase text-amber-700 hover:text-amber-800 bg-amber-500/10 hover:bg-amber-500/20 px-2 py-0.5 rounded-lg border border-amber-500/20 transition-all cursor-pointer shadow-sm active:scale-95"
                          >
                            {isIndo ? '➕ Tambah Semua' : '➕ Add All Boosters'}
                          </button>
                        )}
                      </div>
                      <p className="text-[9px] text-slate-500 dark:text-slate-400 font-bold leading-normal">
                        {isIndo ? event.descriptionIndo : event.description}
                      </p>
                      {eventTagsNotAdded.length > 0 ? (
                        <div className="flex flex-wrap gap-1 pt-1">
                          {eventTagsNotAdded.slice(0, 10).map((tag) => (
                            <button
                              key={tag}
                              type="button"
                              onClick={() => {
                              ...
                              className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold bg-amber-500/10 hover:bg-amber-500/20 text-amber-800 dark:text-amber-300 border border-amber-500/30 transition-all cursor-pointer active:scale-95"
                            >
                              <Plus className="w-2 h-2 mr-0.5 shrink-0" />
                              {tag}
                            </button>
                          ))}
                          {eventTagsNotAdded.length > 10 && (
                            <span className="text-[8px] text-slate-400 flex items-center font-bold px-1 select-none">
                              +{eventTagsNotAdded.length - 10} {isIndo ? 'lainnya' : 'more'}
                            </span>
                          )}
                        </div>
                      ) : (
                        <div className="text-[9px] text-emerald-600 dark:text-emerald-400 font-extrabold flex items-center gap-1 select-none">
                          <Check className="w-3 h-3" />
                          {isIndo ? 'Semua kata kunci musiman aktif sudah ditambahkan! 🌟' : 'All seasonal booster keywords active! 🌟'}
                        </div>
                      )}
                    </div>
                  );
            ...
            {/* Market Search Weight Breakdown (Bobot Kepadatan) */}
            <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1.5">
              <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 block">
                💡 {isIndo ? 'BOBOT KEPADATAN PENCARIAN PASAR' : 'MARKET SEARCH VOLUME DENSITY'}
              </span>
              <div className="grid grid-cols-3 gap-2 pt-0.5">
                <div className="p-1.5 rounded-xl bg-emerald-500/5 border border-emerald-500/15 text-center">
                  <div className="text-[9px] font-black text-emerald-600 dark:text-emerald-400">👑 HIGH VOL</div>
                  <div className="text-[12px] font-mono font-black text-slate-800 dark:text-slate-200">
                    {seoResult.keywordInsights.filter((k) => k.volumeRating === 'HIGH').length}
                  </div>
                </div>
                <div className="p-1.5 rounded-xl bg-violet-500/5 border border-violet-500/15 text-center">
                  <div className="text-[9px] font-black text-violet-600 dark:text-[#a78bfa]">⚡ MEDIUM VOL</div>
                  <div className="text-[12px] font-mono font-black text-slate-800 dark:text-slate-200">
                    {seoResult.keywordInsights.filter((k) => k.volumeRating === 'MEDIUM').length}
                  </div>
                </div>
                <div className="p-1.5 rounded-xl bg-slate-500/5 border border-slate-500/15 text-center">
                  <div className="text-[9px] font-black text-slate-500 dark:text-slate-400">🔹 LONG-TAIL / GENERIC</div>
                  <div className="text-[12px] font-mono font-black text-slate-800 dark:text-slate-200">
                    {seoResult.keywordInsights.filter((k) => k.volumeRating === 'LOW').length}
                  </div>
                </div>
              </div>
            </div>
