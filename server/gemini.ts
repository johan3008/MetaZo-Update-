...
- 'code': Complete, runnable JSX Remotion code containing MotionComposition.`;
  ...
  if (NON_GEMINI_PROVIDERS.has(provider)) {
    ...
    try {
      const res = await callGeminiWithRetry(model?.startsWith('gemini') ? model : 'gemini-3.1-pro-preview', fullContents, { systemInstruction, responseMimeType: "application/json", responseSchema, temperature: 0.85 }, 2);
      responseText = res.text || "{}";
    } catch (err: any) {
