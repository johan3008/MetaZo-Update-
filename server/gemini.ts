...
function hasValidSearchIntent(keyword: string): boolean {
  ...
  // Single words that don't match any pattern may still be valid
  // (they were generated by AI based on visual analysis).
  // Only reject if too short or clearly garbage.
  return k.length >= 3 && !/^[a-z]{1,2}$/.test(k);
}
...
function validateStrictly(keywords: string[], tiers: TieredVisualAnalysis, title?: string): string[] {
  ...
  for (const kw of keywords) {
    ...
    // Allow keywords through if visualRef is empty (no vision data) OR if grounding exists.
    // For keywords with 2+ words, check if at least one word matches visual elements.
    const hasVisualGrounding = visualRef.length === 0 ||
      visualRef.some(v => k.includes(v) || v.includes(k)) ||
      k.split(/\s+/).some(word => word.length > 3 && visualRef.some(v => v.includes(word) || word.includes(v)));

    if (!hasVisualGrounding) {
      // Allow through if keyword is a high-quality compound phrase (3+ words)
      // that likely describes the scene even if no exact visual match found
      if (k.split(/\s+/).length >= 3) {
        // Pass through — long phrases carry scene context
      } else {
        continue;
      }
    }
...
function rankAndWeightKeywords(keywords: string[], tiers: TieredVisualAnalysis, title?: string): string[] {
  // PHASE 3 — RANK COMMERCIALLY ONLY (validation already done in pipeline)
  // Tidak boleh memfilter keyword lagi — hanya urutkan berdasarkan commercial intent.
  const ranked = rankCommercially(keywords, tiers, title);

  // KEYWORD #1 MUST ALWAYS BE MAIN SUBJECT (lock — pastikan di posisi pertama)
  const primarySubject = tiers.objects
    .find(o => o.tier === "primary" || o.importance >= 70)
...
OUTPUT FORMAT:
{
  ...
  try {
    ...
    if (!data.keywords || !Array.isArray(data.keywords)) {
      ...
      // PHASE 2 — VALIDATE STRICTLY: Hanya keyword dengan visual grounding + search intent yang lolos
