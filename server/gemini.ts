...
export const generateOptimizedPrompt = async (options: {
  ...
  const styleSpecificDirectives: Record<string, string> = {
    ...
    "Graphic Design": ' - Act as an expert human graphic designer. Focus on creating a professional, versatile graphic design composition perfectly suited for commercial use (like promotional banners, event posters, sale announcements, product showcases, etc.). Design the background and layout as a ready-to-use, high-end template foundation with a commercial, polished aesthetic. Emphasize ample clean negative space (copy space) specifically intentionally placed for text layout, beautiful typography integration (or placeholders), stylish geometric or 3D graphical elements, modern gradients, and strong visual hierarchy. Include terms like "promotional banner", "typography layout", "graphic design poster", and "commercial advertising aesthetic". The result should look like a human designer crafted it for a professional marketing campaign. AVOID: Pure photorealism, realistic natural photography, or unstylized real-world photographs.'
  };
