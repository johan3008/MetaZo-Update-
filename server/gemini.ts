...
export const generateOptimizedPrompt = async (options: {
  ...
  const styleSpecificDirectives: Record<string, string> = {
    ...
    "Voxel Art": ' - Focus on 3D pixel art constructed from volumetric cubes (voxels). Emphasize a blocky, retro video game aesthetic similar to Minecraft, with low-resolution 3D geometry but modern high-quality lighting (raytracing, global illumination). Use sharp pixelated textures, crisp cube edges, and a rigid grid-based structure. CRITICAL: Do not use the word "Minecraft" or specific game IP; instead use "voxel art", "3D blocky pixel art", or "cubical world".',
    ...
    "Graphic Design": ' - Focus on creating a professional graphic design composition perfectly suited for commercial use, such as promotional banners, event posters, sale announcements, or advertising. Emphasize beautiful typography integration (or typography placeholders), ample clean negative space (copy space) for text, stylish geometric or 3D graphical elements, modern gradients, and strong visual hierarchy. Include terms like "promotional banner", "typography layout", "graphic design poster", and "commercial advertising aesthetic". The composition should serve as a ready-to-use template foundation with a commercial, polished aesthetic.'
  };
